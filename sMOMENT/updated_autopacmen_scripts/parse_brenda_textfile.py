...
# ***edit***
# Modification in this part allows for correct handling of "\t" characters 
# (the original code did replace tabs but did not save the result back to the list)
            for i, part in enumerate(organism_line_split):
                if len(part) > 0:
                    part = part.replace("\t", "")
                    organism_line_split[i] = part
# ***edit***
